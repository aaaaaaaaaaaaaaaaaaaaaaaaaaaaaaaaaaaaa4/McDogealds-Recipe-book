...
def recipe(request, recipeid):
    return render(request, "site/recipe.html", {
        "item": items[int(recipeid) - 1],
        "theitems": items
    })
...
class AddRecipeForm(forms.Form):
    ...
    about = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                'style': 'margin-top: 0px; margin-bottom: 0px; height: 184px;',
                'placeholder': "About"
            }))
    ...
    url = forms.CharField(
        label="", widget=forms.TextInput(attrs={'placeholder': 'Image Url'}))
...
def add(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "site/add.html", {"form": form})
    return render(request, "site/add.html", {"form": AddRecipeForm()})
...
def search(request):
    return render(request, "site/search.html", {"theitems": items})
